...
L'Open Web o il web di superfice rappresenta tutti quei siti web, pubblicamente e facilmente accessibili, questo siccome i normali motori di ricerca sono in grado di indicizzarli.
Esistono in oltre numerosi siti Web inacessibili in quanto la loro indicizzazione non è possible se non con l'utilizzo di browser e sistemi specifici, il loro insime, forma il cosidetto Deep Weeb (o Web invisibile).
...
Come possiamo vedere il web di superfice è solo la punta visibile al di sopra del livello dell'acqua.
Sfortunatamente la maggior parte delle persone (aziende comprese) non competenti in ambito tecnologico o non a conoscenza di certi dettagli e informazioni, permettono l'indicizazzione dei propri dati. Ciò permette ad un'attaccante di raccogliere informazioni e ottenere l'accesso ai propri sistemi, a localizzare dati personali e molto altro.
...
Il dark web è quindi un contenitore di informazioni, basato sul web e caratterizzato da sistemi di comunicazioni anonimi, allo scopo di proteggere utenti e la loro identità attraverso tecnologie, quali le comunicazioni criptate, la crittografia del traffico e il suo offuscamento.
...
Come detto in precedenza il web come lo conosciamo, comprende solo la parte indicizzabile e pubblicamente accessesibile, consultabile tramite Google, Bing, Yahoo e altri motori di ricerca (Questo lato della rete contiene all'incirca 55 miliardi di pagine web).

Può sembrarci un numero davvero elevato, ma il numero di siti web presenti su internet non indicizzabili e non pubblicamente visibili è ancora più elevato, in questa parte della rete possono accedere solo governi, servizi militari e altre oraganizzazioni, ad esempio la NASA, questi servizi sono nascosti dal resto della società e l'insieme di questi contenuti forma il deep Web. Si stima che il deep web sia da 400 a 500 volte più grande del web di superficie.
Il dark web di cui abbiamo parlato finora, fa parte del cosidetto deep web e fornisce anonimato e sicurezza a chiunque la visiti, il dark web come, viene utilizzato per molteplici scopi, sia legali che illegali. Ed è costruita su una serie di reti sovrapposte. \footnote{Queste reti prendono il nome di Overlay Networks}
...
Possiamo quindi affermare che il dark web sia la parte più pericolosa e profonda del deep web, un enorme mondo "sotterraneo" dove alcune delle peggiori persone, come hacker, gangster, terroristi e pedofili, vanno per divertirsi o acquistare qualsiasi cosa.
...
\begin{itemize}
    ...
    \item Gioralisti che risiedono in stati con regimi repressivi usano Tor per comunicare con il mondo esterno e per poter diffondere ingiustizie sociali e le violaizoni dei diritti umani che avvengono in determinati periodi di crisi o conflitto.
    \item Tor permette ad agenti delle forze dell'ordine di navigare e visitare siti e servizi sospetti senza lasciare tracce compromettenti. Questo perchè nel caso in cui un indirizzo IP di un'agenzia governativa o delle forze dell'ordine fosse rilevato nei log del sito, rivelerebbe che il sito è sotto sorveglianza.
...
Per garantire anonimato e sicurezza utilizza una serire di entry poin randomici selezionati da una serie di server e le richieste vengono criptate e inoltrate insieme all'indirizzo di destinazione finale.
...
Usare metodo di pagamento specifici, in particolar modo cryptovalute, quali Bicoin (BTC) e Monero, questo perchè sono le monete più facili da ottenere e scambiare, e perchè Monero può essere acquista solo utilizzando un'altra criptovaluta. 
...
Sarà poi necessario individuare gli URL .onion di interesse, tramite specifiche directory e forum sul dark web, lasciando il minor numero di traccie possibili.
...
\'E fortemente consigliato il collegamento ai soli siti HTTPS. In cui i dati vengono trasmessi in forma crittografata, tramite l'utilizzo di un certificato SSL. TOR non utilizza lo stesso certificato ma questa precauzione è da considerare sul web di superfice.
...
In fine l'utlizzo di software Antivirus.
